...
本目录是一份可 **单独复制到任意 Git 仓库** 的 Cursor Agent Skill：单图/多子图 **标题居中**、**数据区四边闭合框**（`panel.border`）、多子图在 **各子图下方居中** 标 `(a)(b)(c)…`，且 **不要** 整张拼板最外层的「大方框」。

## 目录里有什么（照着做需要看的文件）

| 文件 | 用途 |
|------|------|
| [README.md](README.md) | **人类入口**：安装依赖、一键出图、Cursor 用法（本文件）。 |
| [SKILL.md](SKILL.md) | Cursor 读取的规则正文（**必读摘要**）。 |
